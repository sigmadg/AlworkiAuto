...
import { getSecondary } from '../../../forms/charts/apex-chart/UpdateColors';
  ...
  <v-card elevation="0">
    <v-card variant="outlined">
      <v-card-text>
        ...
        <v-card class="bg-lightsecondary mt-5 overflow-hidden">
          <div class="pa-5">
        ...
        <div class="mt-4">
          <PerfectScrollbar :style="{ height: '270px' }">
            <v-list lines="two" class="py-0">
              <v-list-item v-for="(revenue, i) in revenues" :key="i" :value="revenue" color="secondary" rounded="sm">
                <template #append>
                  <div
                    v-if="revenue.price > 145"
                    class="bg-lightsuccess rounded-sm d-flex align-center justify-center ms-3"
                    style="width: 20px; height: 20px"
                  >
                    <ChevronUpIcon stroke-width="1.5" width="20" class="text-success" />
                  </div>
                  <div v-else class="bg-lighterror rounded-sm d-flex align-center justify-center ms-3" style="width: 20px; height: 20px">
                ...
                <div class="d-inline-flex align-center justify-space-between w-100">
                  ...
                  <div class="ms-auto text-subtitle-1 text-medium-emphasis font-weight-bold">${{ revenue.price }}</div>
                </div>
              </v-list-item>
            </v-list>
          </PerfectScrollbar>
