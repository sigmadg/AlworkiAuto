        ...
        <v-row>
          ...
          <v-col cols="12" lg="4">
            <UiChildCard title="Basic">
              <v-list>
                <v-list-item v-for="(list, i) in list1" :key="i" class="mb-2" :value="list" rounded="md" color="secondary">
                  <template #prepend>
                    <component :is="list.icon" size="20" stroke-width="1.5" class="me-2" />
                  </template>
                ...
                <v-list-group value="Users">
                  <template #activator="{ props }">
                    <v-list-item v-bind="props" rounded="md" color="secondary" title="1st Level with Icon">
                      <template #prepend>
                        <DragDropIcon size="20" stroke-width="1.5" class="me-2" />
                      </template>
                    </v-list-item>
                  </template>
                  <v-list-item value="Nested List" color="secondary" title="Nested List" rounded="md">
                    <template #prepend>
                      <CircleDotIcon size="15" stroke-width="1.5" class="me-2" />
                    </template>
                  </v-list-item>
          ...
          <v-col cols="12" lg="4">
            <UiChildCard title="Selected List Item">
              <v-list>
                <v-list-item v-for="(list, i) in list1" :key="i" class="mb-1" rounded="md" :value="list" :to="list.link" color="secondary">
                  <template #prepend>
                    <component :is="list.icon" size="20" stroke-width="1.5" class="me-2" />
                  </template>
                  <v-list-item-title>{{ list.name }}</v-list-item-title>
          ...
          <v-col cols="12" lg="4">
            <UiChildCard title="Radio Button List">
              <div class="bg-lightprimary rounded-sm px-5 py-3 my-3">
                <v-switch v-model="swt1" label="Start DND Mode" color="primary" hide-details />
                <v-switch v-model="swt2" label="Allow Notifications" color="primary" hide-details />
              </div>
            </UiChildCard>
          </v-col>
          <!-- Folder List -->
          ...
          <v-col cols="12" lg="4">
            <UiChildCard title="Rounded List">
              <v-list>
                <v-list-item v-for="(list, i) in list1" :key="i" :value="list" rounded="md" color="secondary">
                  <template #prepend>
                    <component :is="list.icon" size="20" stroke-width="1.5" class="me-2" />
                  </template>
                  <v-list-item-title>{{ list.name }}</v-list-item-title>
              ...
              <v-list two-lines>
                <template v-for="(custom, i) in customs" :key="i">
                  <v-list-item color="secondary" class="py-3">
                    ...
                    <template #append>
                      <v-list-item-subtitle class="text-subtitle-1 text-high-emphasis me-3"> ${{ custom.price }} </v-list-item-subtitle>
                      <v-chip v-if="custom.profit" color="success" label size="x-small">
                        <ChevronUpIcon size="17" stroke-width="1.5" />
                      </v-chip>
                      <v-chip v-else color="error" label size="x-small">
          ...
          <v-col cols="12" lg="6">
            <UiChildCard title="Scrollable List">
              <PerfectScrollbar style="max-height: 310px">
                <v-list>
                  <v-list-item v-for="(n, index) in 200" :key="index" :value="n" color="secondary">
