...
<template>
  ...
  <v-table>
    <thead>
      <tr>
        <th class="text-start text-subtitle-1">Latest Sales</th>
        <th class="text-start text-subtitle-1">Name Product</th>
        <th class="text-end text-subtitle-1">Price</th>
      </tr>
    </thead>
    <tbody>
      <tr v-for="item in products" :key="item.name">
        <td>{{ item.sales }}</td>
        <td>{{ item.name }}</td>
        <td class="text-end" width="100">$ {{ item.price }}</td>
      </tr>
    </tbody>
  </v-table>
</template>
