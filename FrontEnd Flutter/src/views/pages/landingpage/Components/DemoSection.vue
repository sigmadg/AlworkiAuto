    ...
    <v-container class="maxWidth">
      <v-row class="justify-center">
        <v-col md="7" cols="12" class="text-center">
          <h2 class="text-sm-h1 text-h2 my-3" style="line-height: 1.2">Explore Conceptual Apps</h2>
          <p>Berry has conceptual working apps like Chat, Inbox, E-commerce, Invoice, Kanban, and Calendar</p>
        </v-col>
      </v-row>
    </v-container>
    <v-container fluid>
      <v-row>
        <v-col cols="12">
          <Carousel :wrap-around="true" :breakpoints="breakpoints" class="demo-slider" :autoplay="2000" :transition="500">
            <Slide v-for="(slide, i) in slideShow" :key="i">
              <div class="carousel__item">
                ...
                <v-list class="mt-5 d-inline-block" style="background-color: transparent" variant="text">
                  <v-list-item :href="`${relativeURL}${slide.link}`" target="_blank">
                    <h3>{{ slide.name }}</h3>
                    <template #append>
                      <v-btn variant="text" class="ms-2" size="x-small" icon>
                        <LinkIcon size="14" />
                      </v-btn>
                    </template>
                  </v-list-item>
