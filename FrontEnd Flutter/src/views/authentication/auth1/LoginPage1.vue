...
const component = {
  setup() {
    ...
    const recaptcha = async () => {
      // (opcional) Espera hasta que reCAPTCHA se haya cargado.
      await recaptchaLoaded();

      // Ejecuta reCAPTCHA con la acción "login".
      // Se eliminó la asignación del token ya que no se está utilizando.
      await executeRecaptcha('login');

      // Haz algo con el token recibido.
    };
                  ...
                  <v-card-text class="pa-9">
                    ...
                    <v-row>
                      <v-col cols="12" md="7" sm="7" order-sm="first" order="last" class="text-sm-start text-center">
                        ...
                        <h4 class="text-disabled text-h4">Inicia sesión en tu cuenta</h4>
                      </v-col>
