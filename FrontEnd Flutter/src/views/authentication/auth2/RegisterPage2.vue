<script setup lang="ts">
import Logo from '@/layouts/full/logo/LogoMain.vue';
import AuthRegister from '../authForms/AuthRegister.vue';
import TextSlider from '../auth1/TextSlider.vue';
</script>

<template>
  <v-row class="h-screen" no-gutters>
    <!---Parte Izquierda-->
    <v-col cols="12" lg="7" xl="7" class="d-flex">
      <v-container>
        <Logo />
        <div class="">
          <v-row justify="center">
            <v-col cols="12" lg="10" xl="6" md="7" class="d-flex align-center">
              <v-card elevation="0" class="loginBox">
                <v-card-text class="pa-7">
                  <!---Logo de la Parte Izquierda -->
                  <v-row>
                    <v-col cols="12" class="text-center">
                      <h2 class="text-secondary text-h2">Regístrate</h2>
                      <h4 class="text-disabled font-weight-medium text-h4 mt-4">Ingresa tus credenciales para continuar</h4>
                    </v-col>
                  </v-row>
                  <!---Logo de la Parte Izquierda -->

                  <!---Formulario de la Parte Izquierda-->
                  <AuthRegister />
                  <!---Formulario de la Parte Izquierda-->
                </v-card-text>
              </v-card>
            </v-col>
          </v-row>
        </div>
      </v-container>
    </v-col>
    <!---Parte Izquierda-->
    <!---Parte Derecha-->
    <v-col cols="12" lg="5" xl="5" class="d-none d-lg-flex align-end justify-center bg-lightprimary position-relative">
      <div class="bgpattern">
        <v-container>
          <div class="pa-10">
            <v-row justify="center">
              <v-col cols="12" xl="11">
                <div class="mb-12 pb-14">
                  <TextSlider />
                </div>

                <div class="text-center">
                  <img src="@/assets/images/auth/img-a2-signup.svg" alt="login2" width="300" />
                </div>
              </v-col>
            </v-row>
          </div>
        </v-container>
      </div>
    </v-col>
    <!---Parte Derecha-->
  </v-row>
</template>

<style lang="scss" scoped>
.loginBox {
  max-width: 475px;
  margin: 0 auto;
}

.bgpattern {
  background: url('@/assets/images/auth/img-a2-grid.svg') repeat;
  position: absolute;
  background-position: left bottom;
  background-repeat: no-repeat;
  overflow: hidden;
  margin: 0px 0px 0px auto;
  padding: 50px 0px;
  inset: 0px;
}

.DarkPurpleTheme,
.DarkGreenTheme,
.DarkPinkTheme,
.DarkYellowTheme,
.DarkSeaGreenTheme,
.DarkOliveGreenTheme,
.DarkSpeechBlueTheme,
.v-theme--DarkPurpleTheme {
  .bgpattern {
    background-color: rgb(var(--v-theme-lightprimary));
  }
}
</style>