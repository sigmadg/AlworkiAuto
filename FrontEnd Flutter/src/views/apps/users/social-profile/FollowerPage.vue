          ...
          <v-col cols="12" lg="3" md="4" sm="6" v-for="(card, i) in filteredCards" :key="i">
            <v-card variant="outlined" class="card-hover-border bg-gray100">
              <v-card-text>
                ...
                <div class="mt-5">
                  <v-btn block v-if="card.follow == 1" variant="outlined" color="primary" prepend-icon="$accountMultilineOutline">
                    Followed</v-btn
                  >
                  <v-btn block v-else variant="flat" color="primary" prepend-icon="$accountPlusOutline"> Follow Back</v-btn>
                </div>
              </v-card-text>
            </v-card>
          </v-col>
