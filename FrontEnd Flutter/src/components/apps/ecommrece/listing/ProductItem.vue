...
<template>
  <v-card variant="flat" class="rounded white overflow-hidden">
    ...
    <v-card-text>
      <p class="text-medium-emphasis mb-3 descriptionH">
        {{ desc }}
      </p>
      <div class="text-medium-emphasis align-center d-flex mb-3 ga-2">
        <v-rating v-model="rate" color="warning" half-increments size="small" density="compact" />
        <small>({{ rating }}+)</small>
      </div>
      <div class="d-flex align-center ga-2">
        <h4 class="text-h4">${{ salePrice }}</h4>
        <p class="text-decoration-line-through text-medium-emphasis">${{ offerPrice }}</p>
        <v-btn color="primary" class="ms-auto" size="small" @click="$emit('handlecart', (successsnackbar = true))">
          <BasketIcon size="18" />
        </v-btn>
      </div>
    </v-card-text>
    <v-snackbar v-model="successsnackbar" variant="flat" location="top right" color="success" rounded="md" class="text-white">
