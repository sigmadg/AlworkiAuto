...
import emptyMail from '@/assets/images/img-empty-mail.png';
...
<template>
  <div v-if="!emailDetails">
    ...
    <!---Mail Listing Table-->
    <v-table class="mt-3 mailTable" v-if="filteredMails.length > 0">
      <tbody>
        <tr
        ...
        >
          ...
          <td class="checkboxWidth">
            <v-btn icon size="small" flat @click="handleStarredChange(`${mail.id}`)">
              <v-icon v-if="mail.starred" color="warning" icon="$star"></v-icon>
              <v-icon icon="$starOutline" v-else></v-icon>
            </v-btn>
          </td>
          <td width="30">
            <v-btn icon size="small" flat @click="handleImportantChange(`${mail.id}`)">
              <v-icon v-if="mail.important" color="secondary" icon="$label"></v-icon>
          ...
          <td @click="handleUserChange(mail)">
            <div class="d-flex align-center ga-2">
              ...
              <v-btn icon v-if="mail.attachments.length > 0" size="small" flat><v-icon icon="$attachment"></v-icon></v-btn>
            </div>
          </td>
    ...
    <!---If no mail-->
    <div v-else class="text-center pa-3 d-flex justify-center align-center empty-content">
      <div>
        <img :src="emptyMail" alt="empty mail" class="empty-mail" />
