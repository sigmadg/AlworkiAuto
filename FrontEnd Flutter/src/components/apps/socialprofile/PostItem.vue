  ...
  <v-card variant="flat" class="pa-5">
    <div class="d-flex ga-4 align-center">
      ...
      <v-menu>
        <template v-slot:activator="{ props }">
          <v-btn icon size="small" variant="text" color="secondary" class="ms-auto" v-bind="props"><DotsVerticalIcon size="14" /></v-btn>
        </template>
    ...
    <div class="my-4 mt-5 d-flex ga-4">
      <v-btn
        ...
        variant="text"
        size="small"
        prepend-icon="$thumbUpOutline"
      >
        {{ post?.data && post?.data.likes && post?.data.likes.value }} <span class="d-none d-sm-inline-block">Like</span>
      </v-btn>
      <v-btn color="secondary" variant="text" size="small" prepend-icon="$messageOutline" @click="toggleCommentbox">
        {{ post?.data.comments ? post?.data.comments.length : 0 }}
        <span class="d-none d-sm-inline-block">Comment</span>
      </v-btn>
      <v-menu>
        <template v-slot:activator="{ props }">
          <v-btn icon class="ms-auto" variant="text" size="small" v-bind="props"><ShareIcon size="16" /></v-btn>
        </template>
        ...
        <v-list>
          <v-list-item v-for="(item, index) in shareitems" :key="index" :value="index">
            ...
            <v-list-item-title class="ms-3">{{ item.title }}</v-list-item-title>
          </v-list-item>
        </v-list>
      </v-menu>
    </div>
